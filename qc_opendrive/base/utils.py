...
def road_belongs_to_junction(road: etree._Element) -> bool:
    # road@junction is xs:string, so it is not necessarily numeric. Any value
    # other than the "-1" sentinel identifies a junction the road belongs to.
    road_junction_id = road.get("junction")
    if road_junction_id is None or road_junction_id.strip() == "-1":
        return False
    else:
        return True
